...
export const LanguageSelector: FunctionalComponent<LanguageSelectorProps> = ({ classes }) => {
    ...
    return (
        <select
            key={`lang-${lang}`}
            class={classNames(
                ' md:transform-gpu md:ease-out md:duration-500 md:transition-transform py-1 pr-7 text-sm rounded-sm text-zinc-900 dark:text-zinc-200 bg-zinc-100 dark:bg-zinc-800 border-zinc-800 dark:border-zinc-200 border-2 font-sans hover:border-slate-500  focus:border-slate-500',
                classes,
            )}
